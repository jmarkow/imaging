function fb_template_match(TEMPLATE,varargin)
...
parfor i=1:length(MAT_FILES)
	...
	sound_features=fb_smscore(data.mic_data,data.fs);
...
function TEMPLATE=select_template(DIR)
	...
	TEMPLATE=fb_spectro_navigate(mic_data);
...
for i=1:length(SELECTED_PEAKS)
	...
	data=load(FILENAMES{i},'mic_data');
...
for i=1:length(SELECTED_PEAKS)
	...
	load(FILENAMES{i},'mic_data','sync_data','ttl_data','rising_data','falling_data');
	...
	for j=1:length(SELECTED_PEAKS{i})
		...
		if length(mic_data)>endpoint && startpoint>0
	
			RAW_DATA(:,trial)=mic_data(startpoint:endpoint);
			SYNC_DATA(:,trial)=sync_data(startpoint:endpoint);
			TTL_DATA(:,trial)=ttl_data(startpoint:endpoint);
			RISE_DATA(:,trial)=rising_data(startpoint:endpoint);
			FALL_DATA(:,trial)=falling_data(startpoint:endpoint);
